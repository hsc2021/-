...
def setup_exp(TransportApp="FIFO", logger_level=logging.INFO):
    ...
    requests = [(n1, n5, 0), (n2, n7, 0), (n4, n6, 0), (n4, n7, 0)]
...
class SIMU(MPSimulations):
    def run(self, setting):
        ...
        set_seed(iter_count)
        ret = setup_exp(TransportApp=setting["transport_app"], logger_level=logging.ERROR)
...
if __name__ == "__main__":
    ...
    # ss = SIMU(settings={
    #     "transport_app": ["FIFO", "FAIR", "QTCP3", "EDTCP-1", "ED2", "PS", "PU"],
    # },  aggregate=True, iter_count=5)

    # ss.start()

    # print(ss.get_data())
    # ss.get_data().to_csv("result/result_xtopo_delay50_100games_a0.7_round2.csv")
    # ss.get_raw_data().to_csv("result/result_xtopo_delay50_100games_a0.7_round2_raw.csv")
    print(setup_exp(TransportApp="EDTCP-1", logger_level=logging.INFO))
